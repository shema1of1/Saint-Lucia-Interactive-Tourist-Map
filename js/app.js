...
// 360MAP original presentation layer. Bus routes are intentionally paused while route provenance is reviewed.
...
// When Sherma's original icon PNGs are added under assets/icons/, put their paths here.
// The fallback letter marker remains visible until a category has an approved original icon asset.
const original360Icons={
  ...
  'Getting Around':null,
  Lodging:null,
  'Police Stations':null
};

function markerIcon(category){
  const iconUrl=original360Icons[category];
  if(iconUrl){return L.icon({iconUrl,iconSize:[44,44],iconAnchor:[22,42],popupAnchor:[0,-40]});}
